...
class employeeClass:
    ...
    def create_form(self):
        """Create input form layout for employee details."""
        # ---------- Row 1 ----------
        ...
        Entry(self.root, textvariable=self.var_salary, font=("goudy old style", 15), bg="lightyellow").place(x=620, y=270, width=180)

        # ------------ Buttons -------------
        btn_add = Button(self.root, command=self.add, text="Save", font=("goudy old style", 15), bg="#2196f3", fg="white", cursor="hand2").place(x=900, y=270, width=100, height=28)
        btn_update = Button(self.root, command=self.update, text="Update", font=("goudy old style", 15), bg="#4caf50", fg="white", cursor="hand2").place(x=900, y=310, width=100, height=28)
        btn_delete = Button(self.root, command=self.delete, text="Delete", font=("goudy old style", 15), bg="#f44336", fg="white", cursor="hand2").place(x=1010, y=270, width=100, height=28)
        btn_clear = Button(self.root, command=self.clear, text="Clear", font=("goudy old style", 15), bg="#607d8b", fg="white", cursor="hand2").place(x=1010, y=310, width=100, height=28)
    ...
    def add(self):
        try:
            ...
            password_hash = bcrypt.hashpw(self.var_pass.get().encode(), bcrypt.gensalt())
            ...
            logging.error(f"Error adding employee: {str(ex)}")
            messagebox.showerror("Error", f"Error adding employee: {str(ex)}", parent=self.root)
    ...
    def update(self):
        try:
            ...
            logging.error(f"Error updating employee: {str(ex)}")
    ...
    def delete(self):
        try:
            ...
            logging.error(f"Error deleting employee: {str(ex)}")
    ...
    def show(self):
        try:
            ...
            logging.error(f"Error displaying employees: {str(ex)}")
            messagebox.showerror("Error", f"Error displaying employees: {str(ex)}", parent=self.root)
    ...
    def search(self):
        try:
            ...
            cur.execute(query, (f"%{self.var_searchtxt.get()}%",))
            rows = cur.fetchall()
            if len(rows) == 0:
            ...
            logging.error(f"Error searching employees: {str(ex)}")
            messagebox.showerror("Error", f"Error searching employees: {str(ex)}", parent=self.root)
